...
def buy():
    ...
    if remain < 0:
        return apology("Insufficient Cash. Failed Purchase.", 400)

    # deduct order cost from user's remaining balance (i.e. cash)
    db.execute("UPDATE users SET cash = ? WHERE id = ?", remain, user_id)
    shorten = db.execute("INSERT INTO orders (user_id, symbol, shares, price, timestamp) VALUES (?, ?, ?, ?, ?)",
